...
modules/
  ...
  payment-orders/ # Órdenes de pago; tipos: SERVICIO|MATERIALS|PAYROLL|PETTY_CASH|OFFICE. OFFICE crea OfficeExpense al marcar PAID (no Expense). Non-OFFICE propaga paymentBank+paymentReference al Expense auto-generado
  ...
  monitoring/     # Health check + análisis IA + POST /errors/report (rate-limit 5/hr/IP, auth opcional, graba systemLog y envía email)
services/
...
**PaymentOrder OFFICE — campos requeridos y restricciones:** `orderType === 'OFFICE'` requiere `officeExpenseCategory` y al menos uno de `supplierId` o `officeSupplierName`. `projectId` y `supplierId` son nullable en la tabla (migración `20260626000001`). Al marcar PAID, la rama OFFICE crea un `OfficeExpense` y guarda su id en `officeExpenseId` — la rama de `Expense` normal tiene guarda `po.orderType !== 'OFFICE'`. La función `generateExpenseForOrder` lanza AppError para OFFICE. En `project-suppliers.service.ts` se salta la orden si `!order.supplierId`.
